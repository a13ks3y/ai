      ...
      function fetch2(input, init) {
        return new Promise(function(resolve2, reject) {
          ...
          xhr.onload = function() {
            ...
            setTimeout(function() {
              resolve2(new Response(body, options2));
            }, 0);
          };
...
var fetchWithHeaders = async (fetch2, url, options2 = {}) => {
...
var get = async (fetch2, host, options2) => {
...
var post = async (fetch2, host, data, options2) => {
...
var del = async (fetch2, host, data, options2) => {
  const response = await fetchWithHeaders(fetch2, host, {
    method: "DELETE",
      ...
      panel.webview.onDidReceiveMessage(async (msg) => {
        if (msg.command === "chat") {
          ...
          try {
            ...
            const streamResponse = await index.chat({
              model: modelName,
              stream: true,
              messages
            ...
            const html2 = marked.parse(responseText);
            panel.webview.postMessage({
...
async function buildChatWebViewHtml(context) {
  const htmlPath = import_path2.default.join(context.extensionPath, "src", "chat.html");
  try {
...
async function showPs(modelName) {
  const ps = await index.ps();
